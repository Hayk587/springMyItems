...
public class ItemService {
    ...
    public Item addItemFromItemReuest(CreateItemRequest createItemRequest, MultipartFile[] uploadedFiles) throws IOException {
    ...
    private void saveItemImages(MultipartFile[] uploadedFiles, Item item) throws IOException {
        if (uploadedFiles.length != 0) {
            for (MultipartFile uploadedFile : uploadedFiles) {
                ...
                File newFile = new File(imagaPath + fileName);
    ...
    private List<Category> getCategoriesFromRequest(CreateItemRequest createItemRequest) {
        ...
        for (Integer category : createItemRequest.getCategories()) {
            categories.add(categoryRepasitory.getById(category));
        }
        return categories;
    }
